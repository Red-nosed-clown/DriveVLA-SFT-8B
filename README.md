...
在相同 2,115 条 scene 隔离验证样本上的 v5/v6 完整对照如下：

| Model | Action Acc | Risk Acc | ADE (m) | FDE (m) | Line-like | Pred Curv | Speed MAE (m/s) |
|---|---:|---:|---:|---:|---:|---:|---:|
| v5 SFT | 74.94% | 95.32% | 0.6908 | 1.5071 | 45.67% | 0.0836 | N/A |
| v6 SFT | 75.74% | 95.32% | 0.7031 | 1.5334 | 41.84% | 0.0930 | 0.8506 |

v6 的 Action Accuracy 提升 0.80 个百分点，其中 `SLOW_DOWN` 从 33.18% 提升到
46.33%，但 `TURN_LEFT` 从 82.97% 降到 72.05%。ADE/FDE 分别小幅增加 1.77%/
1.74%，近似直线率下降 3.83 个百分点，预测弯曲度和左右转最终横向位移更接近
GT。目标速度解析率为 100%，MAE 为 0.8506 m/s；STOP 的速度 MAE 为 0.1040 m/s。
因此 v6 的准确表述是“增强减速决策、速度与轨迹形状建模”，不能写成所有指标
全面提升。下一步使用完全相同的 CARLA 场景与 seed 检验这些变化能否降低碰撞率。

完整报告：

- [v6 full 离线评估](results/trainval_v6_safety_full_eval_report.md)
- [v6 full 轨迹几何分析](results/trainval_v6_safety_full_trajectory_geometry.md)
